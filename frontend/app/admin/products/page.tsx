...
export default function AdminProducts() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    api.get("/products?all=true").then(r => r.json()).then(d => setProducts(d.products || [])).finally(() => setLoading(false));
  }, []);
      ...
      <div className="flex flex-wrap justify-between gap-3">
        ...
        <div className="flex gap-2 flex-wrap">
          <div className="flex items-center gap-2 bg-surface border border-border rounded-2xl px-3 py-2 focus-within:border-green transition-colors">
            ...
            <input className="bg-transparent outline-none text-sm w-36 placeholder:text-textDim text-textPri" placeholder="Search..." value={search} onChange={e => setSearch(e.target.value)} />
          </div>
          <button onClick={openCreate} className="flex items-center gap-2 bg-green text-bg px-5 py-2 rounded-2xl font-sans text-sm font-bold uppercase tracking-widest hover:bg-greenLo transition-colors">
            ...
            <div key={p._id} className={`bg-surface border rounded-3xl overflow-hidden transition-all ${p.isActive ? "border-border hover:border-borderHi" : "border-border opacity-60"}`}>
              ...
              <div className="p-4">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => openEdit(p)} className="flex-1 py-1.5 rounded-xl font-sans text-[11px] font-semibold uppercase tracking-wide border border-border text-textSec hover:border-green hover:text-green transition-colors">
                    Edit
                  </button>
                </div>
              </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-surface border border-border rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
              <div className="flex justify-between items-center px-6 py-5 border-b border-border sticky top-0 bg-surface z-10 rounded-t-3xl">
                ...
                <button onClick={close} className="text-textSec hover:text-textPri transition-colors p-1"><span className="ms">close</span></button>
              </div>
              ...
              <form onSubmit={submit} className="p-6 space-y-4">
                  ...
                  <div className="flex flex-wrap gap-3">
                    {form.images.map((img, i) => (
                      <div key={i} className="relative w-16 h-16 rounded-2xl overflow-hidden border border-border">
                        ...
                        <button type="button" onClick={() => setForm(p => ({ ...p, images: p.images.filter((_, j) => j !== i) }))} className="absolute inset-0 bg-black/60 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity">
                    ...
                    <label className={`w-16 h-16 border-2 border-dashed border-border rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:border-green transition-colors ${uploading ? "opacity-50" : ""}`}>
                      {uploading ? <span className="ms text-green animate-spin" style={{fontSize:"20px"}}>progress_activity</span> : (
                ...
                <div><label className={lbl}>Name *</label><input required value={form.name} onChange={e => setForm(p => ({...p, name:e.target.value}))} className={inp} placeholder="e.g. Ghost OG" /></div>
                  ...
                  {[
                    ...
                    <label key={t.k} className="flex items-center gap-3 cursor-pointer">
                      <button type="button" onClick={() => setForm(p => ({...p,[t.k]:!p[t.k as keyof typeof p]}))}
                        ...
                        <span className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-transform ${form[t.k as keyof typeof form] ? "translate-x-6" : "translate-x-1"}`} />
                      </button>
                ...
                <div className="flex gap-3 pt-2">
                  <button type="button" onClick={close} className="flex-1 border border-border text-textSec py-3 rounded-2xl font-sans text-sm font-semibold hover:border-borderHi transition-colors">Cancel</button>
                  <button type="submit" disabled={saving} className="flex-1 bg-green text-bg py-3 rounded-2xl font-sans text-sm font-bold uppercase tracking-widest hover:bg-greenLo transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
