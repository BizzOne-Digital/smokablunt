      ...
      <main className="pt-14">
        ...
        <section className="max-w-site mx-auto px-4 md:px-8 pb-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {[
              { title:"Flowers",      desc:"Artisanal strains, hand-picked for potency and flavour",  href:"/shop?type=flowers",      img:"/pic1.png" },
              { title:"Edibles",      desc:"Precisely dosed chocolates, gummies, and infused treats", href:"/shop?type=edibles",      img:"/pic2.png" },
              { title:"Concentrates", desc:"Premium wax, shatter, and live resin extracts",           href:"/shop?type=concentrates", img:"/pic3.png" },
            ].map(c => (
              <Link key={c.title} href={c.href} className="group relative overflow-hidden rounded-3xl h-72 block">
                <img src={c.img} alt={c.title} className="w-full h-full object-cover opacity-40 group-hover:opacity-60 group-hover:scale-105 transition-all duration-500" />
                <div className="absolute inset-0  from-bg via-bg/30 to-transparent" />
                <div className="absolute bottom-0 left-0 p-6">
                  <p className="font-title text-2xl font-bold text-textPri mb-1">{c.title}</p>
                  <p className="font-sans text-xs text-textSec mb-4 max-w-xs">{c.desc}</p>
                  <span className="inline-flex items-center gap-1.5 bg-green/10 border border-green/30 text-green px-3 py-1.5 rounded-full font-sans text-xs font-semibold group-hover:bg-green group-hover:text-bg transition-all">
                    Shop Now <span className="ms" style={{fontSize:"14px"}}>arrow_forward</span>
                  </span>
                </div>
              </Link>
            ))}
        ...
        <section className="max-w-site mx-auto px-4 md:px-8 pb-20">
          <div className="bg-surface border border-border rounded-3xl p-8 md:p-12 grid md:grid-cols-2 gap-8 items-center overflow-hidden relative">
            ...
            <div className="relative z-10">
              ...
              <p className="font-sans text-sm text-textSec leading-relaxed mb-6">Serving Wyoming, Kingston, Wilkes-Barre and surrounding areas. Delivered in 60–90 minutes. Free delivery on orders over 100.</p>
            ...
            <div className="grid grid-cols-2 gap-3 relative z-10">
              {[
                {icon:"timer",         t:"60–90 Min",       s:"Average delivery"},
                {icon:"local_shipping",t:"Free Delivery",   s:"Orders over 100"},
