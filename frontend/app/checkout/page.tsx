...
export default function CheckoutPage() {
  ...
  const [form, setForm] = useState({
    ...
    email: user?.email || "", phone: "", address: "", city: "", zip: "", notes: "", payment: "cash",
  });
  ...
  const canNext = form.firstName && form.email && form.phone && form.address && form.city && form.zip;
  ...
  const placeOrder = async () => {
    ...
    try {
      const r = await api.post("/orders", {
        ...
        items: items.map(i => ({ product: i.id, name: i.name, image: i.image, price: i.price, quantity: i.quantity })),
    ...
    <>
      ...
      <main className="pt-20 pb-16 max-w-site mx-auto px-4 md:px-8">
        <div className="py-8">
          ...
          <p className="font-sans text-sm text-textSec mt-1">19+ ID required at delivery</p>
        </div>
            ...
            {step === 1 && (
              <div className="bg-surface border border-border rounded-3xl p-6 md:p-8 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div><label className={lbl}>First Name *</label><input required type="text" value={form.firstName} onChange={e => set("firstName", e.target.value)} className={inp} placeholder="John" /></div>
                  <div><label className={lbl}>Last Name</label><input type="text" value={form.lastName} onChange={e => set("lastName", e.target.value)} className={inp} placeholder="Doe" /></div>
                </div>
                <div><label className={lbl}>Email *</label><input required type="email" value={form.email} onChange={e => set("email", e.target.value)} className={inp} placeholder="your@email.com" /></div>
                <div><label className={lbl}>Phone *</label><input required type="tel" value={form.phone} onChange={e => set("phone", e.target.value)} className={inp} placeholder="249-288-4892" /></div>
                <div><label className={lbl}>Street Address *</label><input required type="text" value={form.address} onChange={e => set("address", e.target.value)} className={inp} placeholder="123 Main St" /></div>
                <div className="grid grid-cols-2 gap-4">
                  <div><label className={lbl}>City *</label><input required type="text" value={form.city} onChange={e => set("city", e.target.value)} className={inp} placeholder="Wyoming" /></div>
                  <div><label className={lbl}>ZIP *</label><input required type="text" value={form.zip} onChange={e => set("zip", e.target.value)} className={inp} placeholder="18644" /></div>
                </div>
                <div><label className={lbl}>Notes</label><textarea rows={2} value={form.notes} onChange={e => set("notes", e.target.value)} className={`${inp} resize-none`} placeholder="Buzzer code, gate code..." /></div>

                <div>
                  <label className={lbl}>Payment Method</label>
                  <div className="space-y-3 mt-2">
                    {[{v:"cash",l:"Cash on Delivery",d:"Pay when your order arrives",i:"payments"},{v:"etransfer",l:"E-Transfer",d:"Send to smokablunt4you@gmail.com",i:"swap_horiz"}].map(pm => (
                      <label key={pm.v} className={`flex items-start gap-4 p-4 rounded-2xl border cursor-pointer transition-all ${form.payment === pm.v ? "border-green bg-greenBg" : "border-border hover:border-borderHi"}`}>
                        <input type="radio" name="pay" value={pm.v} checked={form.payment === pm.v} onChange={() => set("payment", pm.v)} className="mt-1" />
                        <span className="ms text-green mt-0.5" style={{ fontSize: "20px" }}>{pm.i}</span>
                        <div><p className="font-sans text-sm font-semibold text-textPri">{pm.l}</p><p className="font-sans text-xs text-textSec">{pm.d}</p></div>
                      </label>
                    ))}
                  </div>
                </div>

                <button disabled={!canNext} onClick={() => setStep(2)} className="w-full bg-green text-bg py-4 rounded-2xl font-sans text-sm font-bold uppercase tracking-widest hover:bg-greenLo transition-colors disabled:opacity-40">
                  Continue to Review →
                </button>
              </div>
            )}
            ...
            {step === 2 && (
              <div className="bg-surface border border-border rounded-3xl p-6 md:p-8 space-y-4">
                ...
                <div className="bg-bg rounded-2xl p-4 space-y-2 font-sans text-sm">
                  <div className="flex justify-between"><span className="text-textSec">Deliver to</span><span className="text-textPri text-right ml-4 truncate max-w-[180px]">{form.address}, {form.city}</span></div>
                  <div className="flex justify-between"><span className="text-textSec">Payment</span><span className="text-textPri">{form.payment === "etransfer" ? "E-Transfer" : "Cash on Delivery"}</span></div>
                </div>
                {form.payment === "etransfer" && (
                  <div className="p-4 bg-greenBg border border-green/20 rounded-2xl">
                    <p className="font-sans text-xs text-green">📧 Send e-transfer to: <strong>smokablunt4you@gmail.com</strong> — use your order number as the message.</p>
                  </div>
                )}
            ...
            {step === 3 && (
              <div className="bg-surface border border-border rounded-3xl p-6 md:p-8 space-y-5">
                ...
                <div className="p-5 bg-greenBg border border-green/20 rounded-2xl">
                  <p className="font-sans text-sm text-textSec leading-relaxed">By placing this order you confirm you are <strong className="text-green">19+ years old</strong> and will provide valid ID at delivery. Total due: <strong className="text-textPri">{total.toFixed(2)}</strong>.</p>
                </div>
                {err && <div className="p-4 bg-errorBg border border-error/20 rounded-2xl"><p className="font-sans text-sm text-error">{err}</p></div>}
                <div className="flex gap-3">
                  ...
                  <button onClick={placeOrder} disabled={saving} className="flex-1 bg-green text-bg py-4 rounded-2xl font-sans text-sm font-bold uppercase tracking-widest hover:bg-greenLo transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
          ...
          <div>
            <div className="sticky top-20 bg-surface border border-border rounded-3xl p-5 space-y-4">
              ...
              <div className="space-y-2.5 max-h-56 overflow-y-auto">
                {items.map(i => (
                  <div key={i.id} className="flex justify-between gap-2 font-sans text-sm">
                    <span className="text-textSec truncate">{i.name} ×{i.quantity}</span>
              ...
              <div className="border-t border-border pt-4 space-y-2">
                ...
                <div className="flex justify-between font-sans text-sm"><span className="text-textSec">Delivery</span><span className={fee === 0 ? "text-green font-semibold" : "text-textPri"}>{fee === 0 ? "FREE" : `${fee}.00`}</span></div>
                <div className="flex justify-between font-title text-xl font-bold border-t border-border pt-3"><span className="text-textPri">Total</span><span className="text-green">{total.toFixed(2)}</span></div>
              </div>
              {totalPrice < 100 && <p className="text-center font-sans text-xs text-textDim bg-bg rounded-2xl py-2.5">Add <span className="text-green font-bold">{(100 - totalPrice).toFixed(2)}</span> more for free delivery</p>}
            </div>
          </div>
        </div>
      </main>
