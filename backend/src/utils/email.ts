...
const FROM    = `"Smokablunt" <${process.env.GMAIL_USER}>`;
const ADMIN   = process.env.CONTACT_RECIPIENT || process.env.GMAIL_USER || "";
...
export async function sendOrderConfirmation(o: {
  ...
  const lines = o.items.map(i =>
    `<tr><td style="padding:6px 0;color:#a3b3a8;font-size:13px;">${i.name} ×${i.qty}</td>
     <td style="padding:6px 0;color:#10b981;font-size:13px;text-align:right;font-weight:700;">${(i.price * i.qty).toFixed(2)}</td></tr>`
  ).join("");
  ...
  <div style="background:#0d2018;border:1px solid #065f46;border-radius:8px;padding:14px;margin-top:16px;">
    <p style="color:#a3b3a8;margin:0;font-size:13px;">⏱ Est. delivery: <strong style="color:#f4f4f4;">60–90 min</strong>. We'll call when on our way. Have valid ID ready.</p>
  </div>`;
...
export async function sendAdminOrderAlert(o: {
  orderNumber: string; customerName: string; customerEmail: string;
  customerPhone: string; items: { name: string; qty: number; price: number }[];
  total: number; address: string; payment: string; notes?: string;
}) {
  if (!process.env.GMAIL_USER || !process.env.GMAIL_APP_PASSWORD) return;
  const lines = o.items.map(i =>
    `<tr>
      <td style="padding:6px 0;color:#a3b3a8;font-size:13px;border-bottom:1px solid #2a3a2e;">${i.name} ×${i.qty}</td>
...
export async function sendContactEmail(d: {
  ...
  if (!process.env.GMAIL_USER || !process.env.GMAIL_APP_PASSWORD) {
    console.log("📧 Contact (no email configured):", d);
    return;
  }
  const body = `
  <table style="width:100%;border-collapse:collapse;">
  ...
  await transporter().sendMail({
    from: FROM, to: ADMIN, replyTo: d.email,
